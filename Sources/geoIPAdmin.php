...
function geoIPFull()
{
	...
	// Init
	$address = 'http://geolite.maxmind.com/download/geoip/database/GeoLiteCity_CSV/GeoLiteCity-latest.zip';
	$filename = $sourcedir . '/geoIP/' . 'GeoLiteCity-latest.zip';

	// This is a ~28M file, can take a while to download if the connection or maxmind site is slow, try to get more time
	// Download the file, approx size is 28M for the status line
	@set_time_limit(900);
	$zipfile = geoIP_fetchdata($address, 28);
		...
		if ($fp)
		{
			...
			if ($extracted !== false)
			{
				...
				if (!empty($extracted) || (file_exists($sourcedir . '/geoIP/GeoLiteCity-Blocks.csv') && file_exists($sourcedir . '/geoIP/GeoLiteCity-Location.csv')))
				{
					...
					if (geoIP_db_full())
						updateSettings(array('geoIP_date' => date("m-d-Y")));
				}
...
function geoIPLite()
{
	...
	// download the file, approx size is 2.3 for the status line
	$zipfile = geoIP_fetchdata($address, 2.3);
